...
- `supplier_order_receptions` : trace chaque réception validée pour garantir
  l'idempotence (contrainte UNIQUE (tenant_id, idempotency_key)).
...
- `receive_supplier_order(p_order_id uuid, p_allocations jsonb, p_idempotency_key text)`
  ...
  fournisseurs est actif). Bloque toute quantité négative ou dépassant le restant
  à recevoir. Pour chaque répartition : met à jour `stock_levels`, crée un
  mouvement d'achat (`movement_type='purchase'`, lié à la commande), et crée un
  lot si le mode lot est actif. Met ensuite à jour `quantity_received` une seule
  fois par ligne, puis le statut de la commande. Toute erreur annule
  l'intégralité de la réception.

## Sécurité
- RLS activée sur `supplier_order_receptions` (SELECT/INSERT authenticated, isolés
  par `current_tenant_id()`).
- Fonction réservée à `authenticated` ; `anon`/`public` révoqués.

## Notes importantes
1. Migration en avant uniquement : aucune réception historique n'est modifiée.
2. Idempotence : un même `idempotency_key` renvoie le résultat sans réappliquer.
3. Mode lot déterminé par `tenants.settings->>'stock_method' = 'lot'`.
4. Prix d'achat, lot et expiration conservés sur le mouvement et le lot.
*/
...
BEGIN
  ...
  -- Contrôle par ligne : pas de dépassement du restant à recevoir
  FOR r_item IN
    SELECT (a->>'item_id')::uuid AS item_id, SUM(COALESCE((a->>'quantity')::numeric, 0)) AS add_qty
    FROM jsonb_array_elements(p_allocations) a
    GROUP BY (a->>'item_id')::uuid
  LOOP
    IF r_item.add_qty < 0 THEN RAISE EXCEPTION 'Quantité négative interdite'; END IF;
    SELECT quantity_ordered, quantity_received INTO v_ord, v_rec
      FROM supplier_order_items WHERE id = r_item.item_id AND order_id = p_order_id AND tenant_id = v_tenant;
    IF v_ord IS NULL THEN RAISE EXCEPTION 'Ligne de commande introuvable'; END IF;
    IF COALESCE(v_rec, 0) + r_item.add_qty > COALESCE(v_ord, 0) THEN
      RAISE EXCEPTION 'Quantité reçue supérieure au restant à recevoir';
    END IF;
  END LOOP;

  -- Appliquer chaque répartition au bon emplacement
  ...
  -- Mettre à jour quantity_received une seule fois par ligne
  FOR r_item IN
    SELECT (a->>'item_id')::uuid AS item_id, SUM(COALESCE((a->>'quantity')::numeric, 0)) AS add_qty
    FROM jsonb_array_elements(p_allocations) a
    GROUP BY (a->>'item_id')::uuid
