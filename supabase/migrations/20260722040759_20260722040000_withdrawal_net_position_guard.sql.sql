/*
# Enforce net position (acompte - debt) limit on customer withdrawals

## Context
A customer may have prepayment credit (acompte) but also an outstanding debt
(customers.balance > 0). Previously, the withdrawal only checked the raw
acompte amount, allowing a client to withdraw more than their true net position.
Example: BALLA DIA has 100,000 acompte and 30,000 debt → net position 70,000.
The client must not be able to withdraw more than 70,000.

## Changes
1. Replaces `record_cash_movement` so the `customer_withdrawal` branch now:
   - Computes available prepayment credit (acompte disponible).
   - Reads the customer's `balance` (outstanding debt) from `customers`.
   - Computes `v_net = v_available - v_balance`.
   - Blocks withdrawal if net <= 0 with a clear message explaining the debt
     covers the acompte.
   - Blocks amounts exceeding the net position with a message showing both the
     acompte and the debt.
2. No schema/column changes — pure function logic update. Safe to re-run.

## Security
- No RLS or policy changes. The function remains SECURITY DEFINER, tenant-scoped
