-- Update both sale functions to also check category-level track_stock
-- An article is considered "non-tracked" if either:
--   1. articles.track_stock = false, OR
--   2. The article's category has part_categories.track_stock = false
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    -- Check article track_stock flag AND category track_stock flag
    SELECT COALESCE(a.track_stock, true), COALESCE(c.track_stock, true)
      INTO v_track_stock, v_cat_track_stock
      FROM articles a
      LEFT JOIN part_categories c ON c.id = a.category_id
      WHERE a.id = (v_item->>'article_id')::uuid;

    -- Skip all stock operations if article OR category has tracking disabled
    IF COALESCE(v_track_stock, true) AND COALESCE(v_cat_track_stock, true) THEN
...
-- Also update create_credit_sale
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    IF v_article_id IS NOT NULL THEN
      -- Check article AND category track_stock flags
      SELECT COALESCE(a.track_stock, true), COALESCE(c.track_stock, true)
        INTO v_track_stock, v_cat_track_stock
        FROM articles a
        LEFT JOIN part_categories c ON c.id = a.category_id
  ...
  UPDATE customers SET balance = COALESCE(balance, 0) + v_total WHERE id = p_customer_id;
