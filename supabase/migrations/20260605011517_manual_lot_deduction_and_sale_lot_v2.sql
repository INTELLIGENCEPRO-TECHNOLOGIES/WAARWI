/*
  # Manual lot deduction RPC

  Accepts a jsonb array of {lot_id, quantity} to deduct manually chosen lots.
  Used by the LotPickerModal when user manually selects lots.
*/

CREATE OR REPLACE FUNCTION deduct_stock_manual_lots(
  p_article_id uuid,
  p_site_id uuid,
  p_total_quantity numeric,
  p_lot_assignments jsonb, -- [{lot_id, quantity}]
...
BEGIN
  ...
  -- Deduct from specified lots
  FOR v_assignment IN SELECT * FROM jsonb_array_elements(p_lot_assignments)
  LOOP
    v_lot_id := (v_assignment->>'lot_id')::uuid;
    v_deduct := (v_assignment->>'quantity')::numeric;
    IF v_deduct > 0 THEN
      UPDATE stock_lots SET remaining_quantity = remaining_quantity - v_deduct
      WHERE id = v_lot_id AND tenant_id = v_tenant_id AND remaining_quantity >= v_deduct;
    END IF;
  END LOOP;

  -- Update stock_levels
  v_new := v_previous - p_total_quantity;
  ...
  VALUES (v_tenant_id, p_article_id, p_site_id, p_movement_type, -p_total_quantity, v_previous, v_new, p_reference_type, p_reference_id, auth.uid(), p_note);
END;
$$;

GRANT EXECUTE ON FUNCTION deduct_stock_manual_lots TO authenticated;

-- Updated create_pos_sale_lot to accept optional lot assignments
-- When p_lot_assignments is provided, use manual selection. Otherwise use FEFO.
...
CREATE OR REPLACE FUNCTION create_pos_sale_lot(
  ...
  p_discount numeric,
  p_note text,
  p_lot_assignments jsonb DEFAULT NULL -- {article_id: [{lot_id, quantity}]}
) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER
SET search_path = public
AS $$
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    IF v_stock_method = 'lot' THEN
      -- Check for manual lot assignments
      v_article_lots := NULL;
