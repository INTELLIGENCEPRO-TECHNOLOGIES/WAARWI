/*
# Instrument sale creation functions to record lot deductions

## Overview
Rewrites create_pos_sale, create_credit_sale, and create_pos_sale_lot to:
1. Set sale_items.site_id so cancel/restore targets the right site
2. Record each lot consumption into sale_lot_deductions for precise restoration

## Modified Functions
- create_pos_sale: now sets sale_items.site_id
- create_credit_sale: now sets sale_items.site_id, records sale_lot_deductions
- create_pos_sale_lot: now sets sale_items.site_id, records sale_lot_deductions

## Important Notes
- Forward-only, idempotent (CREATE OR REPLACE)
- No historical data modified
- Sale items are now inserted BEFORE stock deduction so the sale_item_id
  is available for sale_lot_deductions recording
*/

-- 1. POS normal (no lots) — add site_id to sale_items ----------------------
...
-- 2. Vente à crédit — record lot deductions --------------------------------
...
-- Insert sale_item first to get its id for lot deduction tracking
...
COALESCE((v_item->>'unit_price')::numeric, 0), COALESCE((v_item->>'discount')::numeric, 0), v_line_total, v_purchase_cost, p_site_id)
...
-- Record lot deduction for precise restoration
INSERT INTO sale_lot_deductions (tenant_id, sale_id, sale_item_id, lot_id, article_id, site_id, quantity)
VALUES (v_tenant_id, v_sale_id, v_sale_item_id, v_lot.id, v_article_id, p_site_id, v_deduct);
END LOOP;
END IF;
...
-- Recompute purchase_cost now that we have lot data
IF v_cost_qty > 0 THEN
v_purchase_cost := v_cost_sum / v_cost_qty;
UPDATE sale_items SET purchase_cost = v_purchase_cost WHERE id = v_sale_item_id;
END IF;
END LOOP;
...
-- 3. POS par lots — record lot deductions (explicit + FIFO) -----------------
...
-- Insert sale_item first to get id for lot deduction tracking
SELECT COALESCE(purchase_price, 0) INTO v_purchase_cost FROM articles WHERE id = v_article_id;
v_purchase_cost := COALESCE(v_purchase_cost, 0);
...
-- Record lot deduction
INSERT INTO sale_lot_deductions (tenant_id, sale_id, sale_item_id, lot_id, article_id, site_id, quantity)
VALUES (v_tenant_id, v_sale_id, v_sale_item_id, (v_lot_assign->>'lot_id')::uuid, v_article_id, p_site_id, v_deduct);
END IF;
END LOOP;
ELSE
...
-- Update purchase_cost with weighted lot cost if lots were consumed
IF v_cost_qty > 0 THEN
v_purchase_cost := v_cost_sum / v_cost_qty;
UPDATE sale_items SET purchase_cost = v_purchase_cost WHERE id = v_sale_item_id;
END IF;
