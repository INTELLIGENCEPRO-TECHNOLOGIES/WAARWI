/*
  PC1E-C2-B2.1 — Security and classification fixes for B2

  PART 1: register_supplier_payment — tenant guard on session, payment_type allowlist,
          p_from_cash requires cash type, FOR UPDATE on orders
  PART 2: register_supplier_payment_from_vault — supplier tenant check, PM reject credit/unknown,
          order supplier match, FOR UPDATE on orders
  PART 3: comptabiliser_reglement_fournisseur — order supplier_id match,
          vault movement full coherence (vault_id, supplier_id, site_id, direction, kind)
  PART 4: comptabiliser_reglements_fournisseurs_en_masse — exhaustive category accounting
  PART 5: comptabiliser_depense — FOR UPDATE + tenant filter on UPDATE
  PART 6: Data fix — revert 22 funding_source to NULL, keep only 4 proven cash
*/

-- ============================================================
-- PART 1: register_supplier_payment
-- ============================================================
...
BEGIN
  ...
  IF v_pm.payment_type IS NULL OR v_pm.payment_type NOT IN ('cash','bank','mobile','card','check') THEN
    RAISE EXCEPTION 'Type de paiement non autorisé pour un règlement fournisseur: %', COALESCE(v_pm.payment_type,'NULL');
  END IF;
  IF p_from_cash AND v_pm.payment_type <> 'cash' THEN
    RAISE EXCEPTION 'Paiement depuis la caisse exige un mode de type cash, reçu: %', v_pm.payment_type;
  END IF;
  ...
  IF p_from_cash THEN
    ...
    SELECT cs.id, cs.status, cs.site_id, cs.tenant_id,
    ...
    IF v_session.id IS NULL THEN RAISE EXCEPTION 'Session de caisse introuvable'; END IF;
    IF v_session.tenant_id IS DISTINCT FROM v_tenant_id THEN
      RAISE EXCEPTION 'Session de caisse appartient à un autre tenant';
    END IF;
    IF v_session.status <> 'open' THEN RAISE EXCEPTION 'La caisse doit être ouverte d''abord'; END IF;
    IF NOT EXISTS (SELECT 1 FROM sites WHERE id = v_session.site_id AND tenant_id = v_tenant_id) THEN
      RAISE EXCEPTION 'Site de la session n''appartient pas au tenant';
    END IF;
  ...
  IF p_order_id IS NOT NULL THEN
    SELECT * INTO v_order FROM supplier_orders
      WHERE id = p_order_id AND tenant_id = v_tenant_id AND supplier_id = p_supplier_id
        AND status NOT IN ('cancelled','draft') FOR UPDATE;
    IF v_order.id IS NULL THEN RAISE EXCEPTION 'Commande introuvable ou non éligible'; END IF;
    ...
    IF v_take > 0 THEN
      ...
      UPDATE supplier_orders SET paid = v_new_paid WHERE id = v_order.id AND tenant_id = v_tenant_id;
...
-- ============================================================
-- PART 2: register_supplier_payment_from_vault
-- ============================================================
...
BEGIN
  ...
  IF p_order_id IS NOT NULL THEN
    IF NOT EXISTS (SELECT 1 FROM supplier_orders WHERE id = p_order_id AND tenant_id = v_tenant_id AND supplier_id = p_supplier_id) THEN
      RAISE EXCEPTION 'Commande introuvable dans ce tenant ou fournisseur incohérent';
    END IF;
  END IF;
  ...
  FOR v_order IN SELECT * FROM supplier_orders
    WHERE tenant_id = v_tenant_id AND supplier_id = p_supplier_id AND status NOT IN ('cancelled','draft')
      AND COALESCE(paid,0) < COALESCE(total,0) AND (p_order_id IS NULL OR id <> p_order_id)
...
-- ============================================================
-- PART 3: comptabiliser_reglement_fournisseur
-- ============================================================
...
DECLARE
  ...
  v_order_status text; v_order_acct_status text; v_order_supplier_id uuid;
...
BEGIN
  ...
  IF v_sp.order_id IS NOT NULL THEN
    SELECT so.status, so.accounting_status, so.supplier_id
      INTO v_order_status, v_order_acct_status, v_order_supplier_id
    ...
    IF v_order_supplier_id IS DISTINCT FROM v_sp.supplier_id THEN
      RETURN jsonb_build_object('success', false, 'error', 'Incohérence fournisseur entre règlement et commande');
    END IF;
  ...
  IF v_sp.funding_source = 'vault' THEN
    ...
    IF v_sp.vault_movement_id IS NULL THEN
      RETURN jsonb_build_object('success', false, 'error', 'vault_movement_id manquant');
    END IF;
    IF NOT EXISTS (
      SELECT 1 FROM vault_movements vm
      WHERE vm.id = v_sp.vault_movement_id AND vm.tenant_id = v_sp.tenant_id
        AND vm.vault_id IS NOT DISTINCT FROM v_sp.vault_id
        AND vm.supplier_id IS NOT DISTINCT FROM v_sp.supplier_id
        AND vm.site_id IS NOT DISTINCT FROM v_sp.site_id
        AND vm.direction = 'out'
        AND vm.kind = 'supplier_payment'
    ) THEN
      RETURN jsonb_build_object('success', false, 'error', 'Mouvement coffre incohérent (vault/supplier/site/direction/kind)');
    END IF;
    v_journal := 'CA';
...
-- ============================================================
-- PART 4: comptabiliser_reglements_fournisseurs_en_masse
-- Exhaustive category accounting: skips + eligible = total not_accounted
-- ============================================================
...
-- ============================================================
-- PART 5: comptabiliser_depense — FOR UPDATE + tenant filter
-- ============================================================
...
BEGIN
  ...
  UPDATE cash_movements SET accounting_status = 'accounted', accounting_entry_id = v_entry_id
    WHERE id = p_movement_id AND tenant_id = v_mov.tenant_id;
...
-- ============================================================
-- PART 6: Data fix — revert 22 funding_source to NULL
-- Keep only 4 proven cash-session supplier_payments
-- ============================================================
DO $$
DECLARE
  v_before_cash int; v_before_vault int; v_before_null int; v_total int;
  v_keep_ids uuid[] := ARRAY[
    '308178c8-b9eb-4312-85f2-fe0006efd638'::uuid,
    'f3862e8a-e3d4-4e5f-961e-580f130c55b7'::uuid,
    'ca2771fb-8e3c-4b8b-906d-bb3a3cca0ac5'::uuid,
    '24bd95cc-98c0-4e4b-97a7-23a18d07cb93'::uuid
  ];
  v_updated int;
BEGIN
  SELECT count(*) INTO v_total FROM supplier_payments;
  SELECT count(*) INTO v_before_cash FROM supplier_payments WHERE funding_source = 'cash';
  SELECT count(*) INTO v_before_vault FROM supplier_payments WHERE funding_source = 'vault';
  SELECT count(*) INTO v_before_null FROM supplier_payments WHERE funding_source IS NULL;

  IF v_total <> 34 THEN RAISE EXCEPTION 'Precheck: expected 34 SP, got %', v_total; END IF;
  IF v_before_cash <> 26 THEN RAISE EXCEPTION 'Precheck: expected 26 cash, got %', v_before_cash; END IF;
  IF v_before_vault <> 8 THEN RAISE EXCEPTION 'Precheck: expected 8 vault, got %', v_before_vault; END IF;
  IF v_before_null <> 0 THEN RAISE EXCEPTION 'Precheck: expected 0 null, got %', v_before_null; END IF;

  IF (SELECT count(*) FROM supplier_payments WHERE id = ANY(v_keep_ids) AND funding_source = 'cash') <> 4 THEN
    RAISE EXCEPTION 'Precheck: 4 keep IDs not all cash';
  END IF;

  UPDATE supplier_payments SET funding_source = NULL
    WHERE funding_source = 'cash' AND id <> ALL(v_keep_ids);
  GET DIAGNOSTICS v_updated = ROW_COUNT;
  IF v_updated <> 22 THEN RAISE EXCEPTION 'Expected 22 reverted, got %', v_updated; END IF;

  IF (SELECT count(*) FROM supplier_payments WHERE funding_source = 'cash') <> 4 THEN
    RAISE EXCEPTION 'Post-check: cash count <> 4';
  END IF;
  IF (SELECT count(*) FROM supplier_payments WHERE funding_source IS NULL) <> 22 THEN
    RAISE EXCEPTION 'Post-check: null count <> 22';
  END IF;
  IF (SELECT count(*) FROM supplier_payments WHERE funding_source = 'vault') <> 8 THEN
    RAISE EXCEPTION 'Post-check: vault count <> 8';
  END IF;
END $$;
