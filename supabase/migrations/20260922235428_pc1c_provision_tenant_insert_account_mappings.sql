/*
# PC1C — Add 14 accounting_account_mappings to provision_tenant (9-param overload)

## Purpose
When a new tenant is created via the UI, the 9-parameter provision_tenant function
now also inserts 14 global role→account mappings into accounting_account_mappings,
immediately after provisioning the 17 engine accounts in the accounts table.

## Changes
- Single function replacement: provision_tenant(text,text,text,uuid,text,text,text,text,text)
- Added one DECLARE variable: v_mapping_count integer
- Added one INSERT block: 14 rows into accounting_account_mappings with ON CONFLICT DO NOTHING
- Added one verification block: raises exception if mapping count != 14

## 14 roles provisioned
...
## NOT modified
- Other provision_tenant overloads (2, 3, 4 params)
- All accounting/engine functions
- accounts, accounting_account_mappings data (existing 168 rows)
- payment_methods, journal_entries, journal_lines
- RLS policies, privileges, signatures
- Frontend / UI
*/
...
IF v_mapping_count != 14 THEN
  RAISE EXCEPTION 'PC1C: accounting_account_mappings count is %, expected 14 for tenant %',
