/*
# get_customers_report : exclure ventes supprimées et écritures techniques

Réécrit la fonction de rapport clients pour :
- ignorer les factures supprimées logiquement (`status='deleted'` / `deleted_at`),
  dans le chiffre d'affaires, les marges et le solde à date (CTE sp, ev, comptoir) ;
- exclure les écritures `reconciliation` / `cancel_reversal` du calcul du solde (CTE ev).
