-- Fix: restore SECURITY INVOKER and session ownership check
-- Previous migration incorrectly changed to SECURITY DEFINER, which broke current_tenant_id()
-- Now just adds acomptes to encaissements while keeping original structure intact
...
BEGIN
  ...
  -- Payments received in this session (encaissements from sale_payments)
