/*
  Fix create_pos_sale_lot: column is "name" not "article_name" in sale_items.
  Also fix reset_tenant_data to ensure FK NULLing works with proper exception handling.
*/

-- Drop and recreate create_pos_sale_lot with correct column name
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    INSERT INTO sale_items (sale_id, tenant_id, article_id, name, quantity, unit_price, discount, purchase_cost)
