/*
# Fix source_type and target_type values in credit allocation functions

## Problem
Two functions insert rows into `credit_allocations` with values that violate CHECK constraints:
- `apply_customer_prepayments` uses `source_type = 'prepayment'` but the constraint requires `'prepay'`
- Both `_apply_avoirs_internal` and `apply_customer_prepayments` use `target_type = 'sale'` but the constraint requires `'invoice'`

## Allowed values per CHECK constraints
- `source_type`: 'avoir', 'prepay', 'negative_adjustment', 'customer_payment'
- `target_type`: 'invoice', 'adjustment'

## Changes
- `_apply_avoirs_internal`: change `target_type` from `'sale'` to `'invoice'` (two INSERT statements)
- `apply_customer_prepayments`: change `source_type` from `'prepayment'` to `'prepay'` AND `target_type` from `'sale'` to `'invoice'`
*/

-- Fix _apply_avoirs_internal: target_type 'sale' -> 'invoice'
...
VALUES (p_tenant_id, p_customer_id, 'avoir', v_credit.id, 'invoice', v_sale.id, v_to_apply)
...
-- Fix apply_customer_prepayments: source_type 'prepayment' -> 'prepay', target_type 'sale' -> 'invoice'
...
INSERT INTO public.credit_allocations (tenant_id, customer_id, source_type, source_id, target_type, target_id, amount)
VALUES (v_tenant_id, p_customer_id, 'prepay', v_prepay.id, 'invoice', v_sale.id, v_take)
ON CONFLICT (source_id, target_id, source_type, target_type) DO NOTHING;
...
-- Also fix any existing rows with wrong values so they don't cause issues on re-reads
UPDATE public.credit_allocations SET source_type = 'prepay' WHERE source_type = 'prepayment';
UPDATE public.credit_allocations SET target_type = 'invoice' WHERE target_type = 'sale';
