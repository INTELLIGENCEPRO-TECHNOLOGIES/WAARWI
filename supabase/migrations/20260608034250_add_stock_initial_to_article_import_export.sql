-- Drop and recreate export function with stock_initial column
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(SUM(sl.quantity), 0)::numeric AS stock_initial
  FROM articles a
...
-- Update bulk_import_tenant_articles to handle stock_initial
CREATE OR REPLACE FUNCTION public.bulk_import_tenant_articles(p_rows jsonb)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  -- Resolve default site for stock movements
