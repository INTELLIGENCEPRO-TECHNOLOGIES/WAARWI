/*
  provision_tenant v4 — accepts business_activity_type_id parameter
  
  The new p_activity_type_id parameter allows the tenant's business_activity_type_id
  to be set directly during signup, ensuring the catalog link works immediately.
*/
...
BEGIN
  ...
  -- Resolve activity type ID: use provided one, or look up from business_type slug
  ...
  INSERT INTO tenants (name, email, business_type, business_activity_type_id, approval_status, is_active, status)
  VALUES (p_company_name, v_user_email, coalesce(p_business_type,'auto_parts'), v_resolved_activity_id, 'pending', false, 'pending')
...
-- Grant for new 4-param signature
GRANT EXECUTE ON FUNCTION provision_tenant(text, text, text, uuid) TO authenticated;
