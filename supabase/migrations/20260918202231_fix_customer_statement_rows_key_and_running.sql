/*
# Fix get_customer_statement — restore 'rows' key and 'running' field

## Summary
The previous rewrite accidentally:
1. Changed the JSON key from 'rows' to 'movements' — frontend reads statement.rows
2. Returned raw 'cum' field instead of converting it to 'running' — frontend reads r.running
3. Dropped the `WHERE affects` filter from total_debit/total_credit sums
4. Missing FROM (SELECT 1) _ for the SELECT INTO

This migration restores compatibility with the frontend by:
- Using 'rows' as the JSON key
- Post-processing cum→running with the base offset (same pattern as original)
- Filtering total_debit/total_credit to affects=true rows only
- Using FROM (SELECT 1) _ pattern

## Security
No changes. Function remains STABLE, SECURITY INVOKER.
*/
...
BEGIN
  ...
  WITH movements AS (
    ...
    UNION ALL
    -- Prepayments (tagged as 'prepayment')
