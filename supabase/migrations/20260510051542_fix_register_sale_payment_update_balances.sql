/*
  # Fix register_sale_payment: update session theoretical_amount and customer balance

  1. Changes
    - After inserting the payment, updates `cash_sessions.theoretical_amount`
      when the payment is tied to a session.
    - After updating the sale paid/status, decreases `customers.balance`
      by the payment amount (since the debt is being reduced).

  2. Why
    - Previously invoice payments never reflected in the cash session balance.
    - Previously paying down a credit sale never reduced the customer's
      outstanding balance on the dashboard.
*/
...
BEGIN
  ...
  -- Decrease customer balance (debt is being reduced)
