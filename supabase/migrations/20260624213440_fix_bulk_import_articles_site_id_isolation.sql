-- Fix critical bug: bulk_import_tenant_articles does NOT set site_id on newly created articles
-- This caused imported articles to appear in ALL stores when catalog is independent (shared_articles=false)
-- Also fixes: duplicate detection must respect site_id in independent catalog mode
-- Also fixes: export must filter articles by site_id when provided
...
BEGIN
  ...
  IF p_site_id IS NOT NULL THEN
    ...
    END IF;
  ELSE
    -- In independent catalog mode, site_id is mandatory
    IF NOT v_shared_articles THEN
      RETURN jsonb_build_object('imported', 0, 'updated', 0, 'errors', jsonb_build_array(jsonb_build_object('row', 0, 'error', 'En mode catalogue indépendant, un magasin cible est obligatoire')), 'total', 0);
    END IF;
  ...
  LOOP
    ...
    BEGIN
      ...
      -- Duplicate detection: in independent mode, scope by site_id too
      v_existing_id := NULL;
      IF v_shared_articles THEN
        -- Shared catalog: match by tenant + ref (global uniqueness)
        SELECT id INTO v_existing_id
        FROM articles
        WHERE tenant_id = v_tenant_id AND lower(internal_ref) = lower(v_ref)
        LIMIT 1;
      ELSE
        -- Independent catalog: match by tenant + ref + site_id (per-store uniqueness)
        SELECT id INTO v_existing_id
        FROM articles
        WHERE tenant_id = v_tenant_id AND lower(internal_ref) = lower(v_ref) AND site_id = v_site_id
        LIMIT 1;
        -- Also check for orphaned articles with same ref but NULL site_id (legacy data)
        IF v_existing_id IS NULL THEN
          SELECT id INTO v_existing_id
          FROM articles
          WHERE tenant_id = v_tenant_id AND lower(internal_ref) = lower(v_ref) AND site_id IS NULL
          LIMIT 1;
          -- Claim this orphaned article for the current site
      ...
      IF v_existing_id IS NOT NULL THEN
        ...
        -- Apply stock if target site provided and stock_initial > 0
        ...
        END IF;
      ELSE
        -- INSERT NEW ARTICLE: set site_id in independent catalog mode
...
-- Fix export function to filter by site_id on articles too (not just stock)
...
BEGIN
  ...
  WHERE a.tenant_id = v_tenant_id
    ...
    -- In independent mode with a site filter, only export articles belonging to that site
    AND (
      v_shared_articles = true
