/*
  # Fix tenant_restore_backup: clear accounting FK references before deleting journal_entries

  1. Problem
    - When restoring a backup, the DELETE cascade fails because `sales`,
      `sale_payments`, `supplier_orders`, `supplier_payments`, and `cash_movements`
      have FK references (`accounting_entry_id`) to `journal_entries`.
    - Error: "update or delete on table journal_entries violates foreign key
      constraint sales_accounting_entry_id_fkey on table sales"

  2. Fix
    - Before the delete loop, NULL out all `accounting_entry_id` columns in
      referencing tables for the current tenant.
    - Also reset `accounting_status` to 'not_accounted' since the entries are
      being wiped and will be re-inserted from backup data.

  3. Security
    - No scope change: SECURITY DEFINER, tenant-scoped only.
*/
