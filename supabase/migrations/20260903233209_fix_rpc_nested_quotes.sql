-- Fix: use dollar-quoting for nested string literals in totals queries
...
BEGIN
  ...
  -- Use format() for the totals query to avoid nested-quote issues
  v_totals := jsonb_build_object(
    'sum_total', 0, 'sum_paid', 0, 'count_paid', 0, 'count_credit', 0, 'count_cancelled', 0
  );
  EXECUTE format(
...
BEGIN
  ...
  v_totals := jsonb_build_object('sum_total', 0, 'count_draft', 0, 'count_accepted', 0, 'count_converted', 0);
  EXECUTE format(
    'SELECT jsonb_build_object(
      ''sum_total'', COALESCE(sum(total), 0),
...
BEGIN
  ...
  v_totals := jsonb_build_object('sum_total', 0, 'count_pending', 0, 'count_approved', 0);
  EXECUTE format(
    'SELECT jsonb_build_object(
      ''sum_total'', COALESCE(sum(total), 0),
