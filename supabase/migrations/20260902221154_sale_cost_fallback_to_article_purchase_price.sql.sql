/*
  # Sécurisation du coût d'achat sur les ventes

  Les fonctions de vente enregistraient purchase_cost = 0 lorsque le point de
  vente ne transmettait pas de coût (ex. article sans lot de stock au moment de
  la vente), ce qui faussait la marge à la hausse.

  Cette migration ajoute un garde-fou : si le coût transmis est nul ou absent,
  on retombe automatiquement sur le prix d'achat renseigné dans la fiche article.
  Aucune autre logique n'est modifiée.
*/

CREATE OR REPLACE FUNCTION public.create_pos_sale_lot(p_site_id uuid, p_cash_session_id uuid, p_customer_id uuid DEFAULT NULL::uuid, p_items jsonb DEFAULT '[]'::jsonb, p_payments jsonb DEFAULT '[]'::jsonb, p_discount numeric DEFAULT 0, p_note text DEFAULT ''::text, p_lot_assignments jsonb DEFAULT NULL::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.create_credit_sale(p_customer_id uuid, p_items jsonb, p_discount numeric DEFAULT 0, p_site_id uuid DEFAULT NULL::uuid, p_cash_session_id uuid DEFAULT NULL::uuid, p_note text DEFAULT ''::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
v_purchase_cost := COALESCE((v_item->>'purchase_cost')::numeric, 0);
IF v_purchase_cost <= 0 AND v_article_id IS NOT NULL THEN
