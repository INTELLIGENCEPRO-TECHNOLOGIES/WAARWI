/*
# Fix: exclude generated columns from restore/import INSERT using dynamic column lists

## Problem
`INSERT INTO ... SELECT * FROM jsonb_populate_recordset(...)` fails on tables
with GENERATED ALWAYS columns (e.g. `cash_control_lines.difference_amount`)
because Postgres forbids inserting non-DEFAULT values into generated columns.

## Solution
Replace every `INSERT ... SELECT *` in `br_restore_backup`, `br_import_payload`
with a dynamically-built column list derived from `pg_attribute` that:
- keeps only columns present in BOTH the payload and the current table
- excludes ALL generated columns (`attgenerated <> ''`)
- excludes deferred FK columns on the first pass (restored in second pass)
- preserves normal `id` columns to maintain referential integrity
- lets new columns absent from the backup receive their DEFAULT value

The same helper function `_br_insertable_columns` is used by both functions.

## Preflight update
`br_preflight_restore` now recognises generated columns as non-insertable and
excludes them from the NOT-NULL-without-default check — a generated column that
is NOT NULL is fine because the database computes its value.

## Changes
- New helper: `_br_insertable_columns(schema, table, payload_keys, deferred_cols)`
- Rewritten INSERT paths in `br_restore_backup`
- Rewritten INSERT paths in `br_import_payload`
- Updated `br_preflight_restore` to exclude generated columns from NOT NULL check

## Notes
- No data modifications, no tenant changes, no activation lock changes
- Forward-only migration
- Generic: works for any current or future generated column
*/

-- ===========================================================================
-- 1. Helper: compute insertable column list for a table
-- ===========================================================================
CREATE OR REPLACE FUNCTION _br_insertable_columns(
  p_schema text,
  p_table text,
  p_payload_keys text[],
  p_deferred_cols text[]
)
RETURNS text[]
LANGUAGE plpgsql
STABLE
...
BEGIN
  ...
  INTO v_cols
  FROM pg_attribute a
  WHERE a.attrelid = (p_schema || '.' || p_table)::regclass
    AND a.attnum > 0
    AND NOT a.attisdropped
    AND a.attgenerated = ''
    AND a.attname::text = ANY(p_payload_keys)
    AND a.attname::text != ALL(coalesce(p_deferred_cols, '{}'));
  RETURN coalesce(v_cols, '{}');
END;
$$;

-- ===========================================================================
-- 2. Rewrite br_restore_backup — dynamic column INSERT
-- ===========================================================================
...
-- ===========================================================================
-- 3. Rewrite br_import_payload — dynamic column INSERT
-- ===========================================================================
...
BEGIN
  ...
  -- Insert in forward order — first pass with dynamic column list
  FOR v_rec IN
    SELECT r.schema_name, r.table_name, r.self_ref_columns, r.deferred_fk_columns
    FROM _br_table_registry r
    WHERE r.tenant_link != 'excluded'
...
-- ===========================================================================
-- 4. Update br_preflight_restore — exclude generated cols from NOT NULL check
-- ===========================================================================
...
BEGIN
  ...
  FOR v_rec IN
    ...
    ORDER BY r.restore_order
  LOOP
    -- Get non-generated columns only for comparison
    ...
    -- Collect generated columns for exclusion from NOT NULL check
    ...
    -- NOT NULL without default check — exclude generated columns
    SELECT array_agg(a.attname::text)
