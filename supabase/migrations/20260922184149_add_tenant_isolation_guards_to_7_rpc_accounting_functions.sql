/*
# C2 — Isolation stricte des tenants sur les 7 RPC comptables publiques

## Description
Ajoute un bloc de garde au debut de chacune des 7 fonctions RPC comptables
accessibles aux utilisateurs authentifies. Ces gardes verifient que :
1. L'appelant est authentifie (auth.uid() IS NOT NULL)
2. current_tenant_id() n'est pas NULL
3. Pour les 6 fonctions avec p_tenant_id : p_tenant_id = current_tenant_id()
4. Pour comptabiliser_vente(p_sale_id) : la vente chargee appartient au tenant de l'appelant

## Fonctions modifiees (7)
- comptabiliser_vente(p_sale_id uuid) — filtre tenant sur le SELECT de la vente
- comptabiliser_ventes_en_masse(p_tenant_id uuid) — garde p_tenant_id = current_tenant_id()
- comptabiliser_reglements_clients_en_masse(p_tenant_id uuid) — idem
- comptabiliser_achats_en_masse(p_tenant_id uuid) — idem
- comptabiliser_reglements_fournisseurs_en_masse(p_tenant_id uuid) — idem
- cloturer_journal(p_tenant_id uuid, p_journal_type text, p_date_to date) — idem
- cloturer_exercice(p_tenant_id uuid, p_fiscal_year integer) — idem

## Securite
- Aucun changement de privileges (C1 reste intact)
- Aucun changement de SECURITY DEFINER / search_path
- Aucune modification de la logique metier existante
- Les 5 fonctions internes ne sont PAS touchees

## Notes
1. La garde utilise current_tenant_id() qui est SECURITY DEFINER et retourne NULL si non authentifie
2. Le message d'erreur est generique : 'Acces refuse au tenant demande'
3. Pour comptabiliser_vente, le filtre tenant est ajoute dans le WHERE du SELECT initial
4. Migration idempotente : CREATE OR REPLACE
*/

-- =================================================================
-- 1. comptabiliser_vente(p_sale_id uuid)
--    Garde: auth.uid() + filtre tenant sur le SELECT de la vente
-- =================================================================
...
BEGIN
-- C2 tenant guard
IF auth.uid() IS NULL OR current_tenant_id() IS NULL OR p_tenant_id != current_tenant_id() THEN
  RAISE EXCEPTION 'Acces refuse au tenant demande';
END IF;
