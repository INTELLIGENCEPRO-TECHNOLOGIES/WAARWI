/*
  Include Money Transfer tables in:
  1. reset_tenant_data - full tenant reset
  2. tenant_reset_operations - operations-only reset
  3. tenant_create_backup - backup payload
  4. tenant_restore_backup - restore from backup
*/

-- 1) RESET TENANT DATA - add MT tables (children first)
...
-- 2) RESET OPERATIONS - add MT operational tables (keeps services/points config, wipes operations + resets balances)
...
DECLARE
  ...
  v_ops text[] := ARRAY[
    -- Money Transfer operational data (keep mt_services, mt_service_points, mt_service_point_services, mt_wholesalers)
...
DECLARE
  ...
  v_tables text[] := ARRAY[
    ...
    'balance_adjustments',
    -- Money Transfer
    'mt_services','mt_service_points','mt_wholesalers','mt_service_point_services','mt_accounts','mt_operations','mt_closures',
    -- operational
...
-- 4) RESTORE BACKUP - add MT tables
