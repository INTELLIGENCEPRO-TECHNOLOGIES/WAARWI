/*
  # Include balance_adjustments in reset, backup/restore, and safe-delete

  1. reset_tenant_data: adds balance_adjustments to delete list + resets balance column
  2. tenant_reset_operations: adds balance_adjustments + resets balance column
  3. tenant_create_backup: adds balance_adjustments to backup
  4. tenant_restore_backup: adds balance_adjustments to restore
  5. tenant_delete_customer_safe: deletes balance_adjustments for the customer before deleting
  6. tenant_delete_supplier_safe: deletes balance_adjustments for the supplier before deleting
*/

-- 1) RESET TENANT DATA - add balance_adjustments and reset balance columns
...
-- 2) RESET OPERATIONS (keeps articles/customers/suppliers but wipes operations + resets balances)
...
-- 3) CREATE BACKUP - add balance_adjustments
...
-- 4) RESTORE BACKUP - add balance_adjustments
...
-- 5) DELETE CUSTOMER SAFE - also delete balance_adjustments + check cash_movements
...
BEGIN
  ...
  SELECT
    ...
    + (SELECT count(*) FROM public.cash_movements WHERE customer_id = p_id AND tenant_id = v_tenant)
  INTO v_refs;
  ...
  -- Delete balance adjustments and exception prices before deleting the customer
  DELETE FROM public.balance_adjustments WHERE entity_id = p_id AND entity_type = 'customer' AND tenant_id = v_tenant;
  DELETE FROM public.customer_exception_prices WHERE customer_id = p_id AND tenant_id = v_tenant;
...
-- 6) DELETE SUPPLIER SAFE - also delete balance_adjustments
...
BEGIN
  ...
  -- Delete balance adjustments before deleting the supplier
  DELETE FROM public.balance_adjustments WHERE entity_id = p_id AND entity_type = 'supplier' AND tenant_id = v_tenant;
