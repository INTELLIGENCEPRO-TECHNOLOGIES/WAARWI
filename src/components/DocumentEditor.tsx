            ...
            {validItems.map((it, vIdx) => {
              ...
              return (
                ...
                >
                  ...
                  {canEdit && (
                    <div className="w-8 shrink-0 text-center">
                      <button onClick={e => { e.stopPropagation(); removeItem(vIdx); }} className="p-0.5 rounded text-[var(--w-text-disabled)] hover:text-[var(--w-error)] hover:bg-[var(--w-hover)] opacity-0 group-hover:opacity-100 transition-all"><Trash2 className="w-3 h-3" /></button>
                    </div>
                  )}
                </div>
              );
  ...
  return (
    <div className={`flex items-center gap-0.5 shrink-0 ${compact ? 'flex-wrap justify-end' : ''}`}>
      ...
      {isView && editingId && (
        <>
          ...
          {onPay && docType === 'invoice' && (invoiceDue ?? 0) > 0 && notCancelled && (
            <button onClick={onPay} className={`${btnCls} text-emerald-500 hover:bg-[var(--w-hover)]`} title="Régler"><Coins className="w-3 h-3" />{!compact && ' Régler'}</button>
          )}
          {false && onComptabiliser && docType === 'invoice' && notAccounted && notCancelled && (
            <button onClick={onComptabiliser} className={`${btnCls} text-teal-500 hover:bg-[var(--w-hover)]`} title="Comptabiliser"><BookOpen className="w-3 h-3" />{!compact && ' Comptabiliser'}</button>
          )}
          ...
          {onWhatsApp && (
            <button onClick={onWhatsApp} className={`${btnCls} text-[#25D366] hover:bg-[var(--w-hover)]`} title="WhatsApp"><MessageCircle className="w-3 h-3" />{!compact && ' WhatsApp'}</button>
          )}
          {onCancel && notCancelled && notAccounted && (
            <button onClick={onCancel} className={`${btnCls} text-rose-500 hover:bg-[var(--w-hover)]`} title="Annuler"><Ban className="w-3 h-3" />{!compact && ' Annuler'}</button>
          )}
          {onDelete && notAccounted && (
            <button onClick={onDelete} className={`${btnCls} text-red-500 hover:bg-[var(--w-hover)]`} title="Supprimer"><Trash2 className="w-3 h-3" />{!compact && ' Supprimer'}</button>
          )}
        </>
      )}
      ...
      {docType === 'return' && isView && editingId && documentStatus === 'pending' && (
        <>
          {onRefundCash && (
            <button onClick={onRefundCash} className={`${btnCls} text-emerald-500 hover:bg-[var(--w-hover)]`} title="Rembourser"><Coins className="w-3 h-3" />{!compact && ' Rembourser'}</button>
          )}
          {onApproveAvoir && (
                ...
                <div key={idx} className={`flex items-center gap-2 px-2 py-2 border-b border-neutral-100 transition-colors ${it.selected ? 'bg-neutral-50/50' : ''}`}>
                  ...
                  {it.selected && (
                    <div className="flex items-center gap-1 shrink-0">
                      <button type="button" onClick={() => setQty(idx, it.quantity - 1)} className="w-6 h-6 rounded bg-[var(--w-bg-secondary)] flex items-center justify-center hover:bg-[var(--w-hover)]"><Minus className="w-3 h-3" /></button>
                      ...
                      <button type="button" onClick={() => setQty(idx, it.quantity + 1)} className="w-6 h-6 rounded bg-[var(--w-bg-secondary)] flex items-center justify-center hover:bg-[var(--w-hover)]"><Plus className="w-3 h-3" /></button>
                    </div>
                  )}
    ...
    >
      ...
      <div className="flex items-center gap-2 mt-2">
        {autoMode && onVehiclePicker && (
          <button onClick={() => onVehiclePicker(null)} className="inline-flex items-center gap-1 text-[10px] font-medium px-2 py-1 rounded bg-[var(--w-bg-secondary)] text-[var(--w-text-secondary)] hover:bg-[var(--w-hover)] transition-colors"><Car className="w-3 h-3" />Par véhicule</button>
        )}
        {onValidate && (
        ...
        <div className="absolute z-50 left-0 right-0 top-full mt-1 bg-white border border-neutral-200 rounded shadow-lg max-h-60 overflow-y-auto">
          ...
          {onCreateNew && query.trim() && (
            <button
              ...
              className="w-full text-left px-3 py-1.5 text-xs text-teal-500 font-medium border-t border-[var(--w-separator)] hover:bg-[var(--w-hover)]"
            >
              + Créer "{query}"
            </button>
          )}
                  ...
                  {payments.map((p, i) => (
                    <div key={i} className="flex items-center justify-between px-1 py-1.5 text-[11px]">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => setPayments((prev: any[]) => prev.filter((_: any, j: number) => j !== i))} className="p-0.5 rounded hover:bg-[var(--w-hover)] text-[var(--w-text-muted)] hover:text-red-500"><X className="w-3 h-3" /></button>
                      </div>
                    </div>
                  ))}
                </div>
