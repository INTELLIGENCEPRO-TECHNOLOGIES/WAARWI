    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button onClick={comptabiliserReglements} disabled={bulkPayBusy} className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold bg-blue-50 text-blue-700 border border-blue-200 hover:bg-blue-100 transition disabled:opacity-50">
                ...
                <tbody className="divide-y divide-slate-100">
                  {data.map((r: any) => (
                    <tr key={r.account_code} className="hover:bg-slate-50/60">
                      ...
                      <td className="px-4 py-2.5 text-slate-700">{r.account_name}</td>
    ...
    <div className="space-y-4">
      ...
      {loading ? <div className="py-16 flex justify-center"><Loader2 className="w-6 h-6 animate-spin text-brand-700" /></div>
        : groupedByAccount.length === 0 ? (
          <div className="card py-12 text-center text-sm text-slate-500">Aucun mouvement pour la période sélectionnée.</div>
        ) : (
          <div className="space-y-4">
            {groupedByAccount.map(group => {
              ...
              return (
                <div key={group.code} className="card overflow-hidden">
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead className="text-[10px] uppercase text-slate-500 bg-white">
                        <tr>
                      ...
                      <tbody className="divide-y divide-slate-50">
                        {group.lines.map((l: any, idx: number) => (
                          <tr key={idx} className="hover:bg-slate-50/40">
                            <td className="px-4 py-2 text-xs text-slate-600">{formatDate(l.entry_date)}</td>
                            <td className="px-4 py-2 font-mono text-xs text-slate-500">{l.entry_number}</td>
                            <td className="px-4 py-2"><span className="inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-bold bg-slate-100 text-slate-600">{l.journal_type}</span></td>
                            <td className="px-4 py-2 text-slate-700 text-xs">{l.label || l.reference || '—'}</td>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div className="max-h-[60vh] overflow-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-slate-50">
                  {tiers.length === 0 && (
                    <tr><td colSpan={5} className="px-4 py-8 text-center text-slate-400">Aucun tiers trouvé</td></tr>
                  )}
                  {tiers.map((t: any) => (
                    <tr key={t.account_code} onClick={() => loadExtrait(t)} className={`cursor-pointer hover:bg-brand-50/40 transition ${selected?.account_code === t.account_code ? 'bg-brand-50' : ''}`}>
                      ...
                      <td className="px-3 py-2 text-slate-700 truncate max-w-[140px]">{t.tiers_name}</td>
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            {!selected ? (
              <div className="flex items-center justify-center py-16 text-sm text-slate-400">Cliquez sur un tiers pour voir son extrait</div>
            ) : extraitLoading ? (
              <div className="flex items-center justify-center py-16"><Loader2 className="w-5 h-5 animate-spin text-brand-700" /></div>
            ) : (
              <div className="max-h-[60vh] overflow-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {extrait.length === 0 && (
                      <tr><td colSpan={6} className="px-4 py-8 text-center text-slate-400">Aucune écriture</td></tr>
                    )}
                    {extrait.map((e: any, idx: number) => (
                      <tr key={idx} className="hover:bg-slate-50/40">
                        <td className="px-3 py-2 text-xs text-slate-600">{formatDate(e.entry_date)}</td>
                        <td className="px-3 py-2 font-mono text-xs text-slate-500">{e.entry_number}</td>
                        <td className="px-3 py-2 text-slate-700 text-xs">{e.label || '—'}</td>
    ...
    <div className="space-y-4">
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          <input type="text" placeholder="Texte libre (libellé, pièce...)" value={filters.text} onChange={e => setFilters(f => ({ ...f, text: e.target.value }))} className="rounded-lg border border-slate-200 px-3 py-2 text-sm focus:ring-2 focus:ring-brand-200 focus:border-brand-400 outline-none" />
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-50">
                {results.map((r: any, idx: number) => (
                  <tr key={idx} onClick={() => setDetail(r)} className="hover:bg-slate-50/60 cursor-pointer">
                    <td className="px-4 py-2 text-xs text-slate-600">{formatDate(r.entry_date)}</td>
                    <td className="px-4 py-2 font-mono text-xs text-slate-500">{r.entry_number}</td>
                    <td className="px-4 py-2"><span className="inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-bold bg-slate-100 text-slate-600">{r.journal_type}</span></td>
                    <td className="px-4 py-2 font-mono text-xs text-brand-700">{r.account_code}</td>
                    <td className="px-4 py-2 text-slate-700 text-xs max-w-[200px] truncate">{r.label || r.reference || '—'}</td>
